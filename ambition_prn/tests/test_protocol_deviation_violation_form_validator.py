...
class TestDeviationViolationFormValidator(TestCase):
    ...
    def test_report_type(self):
        ...
        for field_item in field_required_list:
            ...
            cleaned_data = {
                'report_type': DEVIATION, field: value}
    ...
    def test_violation(self):
        ...
        for field_item in field_required_list:
            field, value = field_item
            cleaned_data = {
                'report_type': VIOLATION, field: value}
            form_validator = ProtocolDeviationViolationFormValidator(
                cleaned_data=cleaned_data)
            self.assertFalse(form_validator._errors)
    ...
    def test_yes_safety_impact_none_details(self):
        ...
        cleaned_data = {'safety_impact': YES,
                        'safety_impact_details': None}
        form_validator = ProtocolDeviationViolationFormValidator(
            cleaned_data=cleaned_data)
        self.assertRaises(ValidationError, form_validator.validate)
        self.assertIn(
            'safety_impact_details', form_validator._errors)
    ...
    def test_other_protocol_violation_none_other_protocol_violation(self):
        cleaned_data = {'violation_type': OTHER,
                        'violation_type_other': None}
        form_validator = ProtocolDeviationViolationFormValidator(
            cleaned_data=cleaned_data)
        self.assertRaises(ValidationError, form_validator.validate)
        self.assertIn(
            'violation_type_other', form_validator._errors)
    ...
    def test_corrective_action_datetime(self):
        cleaned_data = {'corrective_action_datetime': get_utcnow(),
                        'corrective_action': None}
        form_validator = ProtocolDeviationViolationFormValidator(
            cleaned_data=cleaned_data)
        self.assertRaises(ValidationError, form_validator.validate)
        self.assertIn(
            'corrective_action', form_validator._errors)
